---
name: 손익계산서 컬럼 및 월말예상 로직 개선
overview: 손익계산서 테이블의 컬럼 구조를 변경하고, Tag매출/실판(V+)/실판(V-)/매출원가의 월말예상 계산 로직을 하위 채널별 라인 기준으로 재구성합니다. 전년 누적 및 전년 진척률 컬럼을 추가하고, 직영/대리상별로 다른 월말예상 계산 방식을 적용합니다.
todos: []
---
